...
class FollowerList extends Component{
    ...
    fetchData = () => {
        ...
        const { id, friend, settings, username } = this.props.route.params
        if (!friend && !settings) {
            axios.get(`${baseURL}/api/follower/list/?limit=10&offset=${data.length}&id=${id}`, fetchToken(this.props.token))
            .then(res => {
                this.setState(state => ({ data: [...res.data, ...state.data], loading: false }))
                res.data < 10 ? this.setState({ no_more: true }) : null
            }).catch(err => this.setState({ error: true, loading: false }))
        } else if (friend && !settings) {
            axios.get(`${baseURL}/api/friends/list/?limit=10&offset=${data.length}&id=${id}`, fetchToken(this.props.token))
            .then(res => {
                this.setState(state => ({ data: [...res.data, ...state.data], loading: false }))
                res.data < 10 ? this.setState({ no_more: true }) : null
            }).catch(err => this.setState({ error: true, loading: false }))
        } else if (!friend && settings) {
            axios.get(`${baseURL}/api/following/list/?limit=10&offset=${data.length}&user=${username}`, fetchToken(this.props.token))
            .then(res => {
                this.setState(state => ({ data: [...res.data, ...state.data], loading: false }))
                res.data < 10 ? this.setState({ no_more: true }) : null
            }).catch(err => this.setState({ error: true, loading: false }))
    ...
    render() {
        ...
        const { color, background_color, third_color } = this.props.route.params.profile
        ...
        return(
            <View style={styles.container}>
                {loading ? null : <HeaderView navigation={navigation} color={color} title={"People"}/>}
...
export default connect(mapStateToProps, { fetchToken } )(FollowerList)